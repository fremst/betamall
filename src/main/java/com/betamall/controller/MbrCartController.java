...
public class MbrCartController extends HttpServlet {
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (session.getAttribute("IpOrd") != null) {
			...
			TreeMap<ArrayList<Integer>, Integer> cart = (TreeMap<ArrayList<Integer>, Integer>) session
					.getAttribute("cart");
			...
			if (cart != null) {
				...
				for (int i = 0; i < ordBrList.size(); i++) {
					if ((ordItemKindsPerBr.size() == 0) || !(ordBrList.get(i - 1).equals(ordBrList.get(i)))) {
						...
						ordItemKindsPerBr.set(ordItemKindsPerBr.size() - 1,
								ordItemKindsPerBr.get(ordItemKindsPerBr.size() - 1) + 1);
					}
