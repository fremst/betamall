...
<head>
    <title>회원가입</title>
    <link rel="stylesheet" href="${cp}/resources/css/joinForm.css">
...
//const form  = document.userInfo;
    ...
    function checkEmail() {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
                exists1 = JSON.parse(xhr.responseText).exist;
                if (exists1 == 1) {
                    alert('사용중인 이메일입니다.');
                } else if (!email) {
                    alert("이메일을 입력하세요.");
        ...
        xhr.open('post', "${cp }" + '/emailcheck?email=' + email, true);
        xhr.send();
    }
</script>
<body>
...
<div id="joinWrap">
    <form action="${cp }/join" method="post" name="userInfo" id="userInfo" onsubmit="return checkValue()">
        <fieldset id=joinField>
            ...
            <input type="button" onclick="sample6_execDaumPostcode()" name="postno" value="우편번호 찾기"><br>
