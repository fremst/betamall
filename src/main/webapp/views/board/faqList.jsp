...
<body>
	...
	<table border="1" width="750">
		...
		<c:forEach var="dto" items="${list }">
			<tr>
				<td>${dto.brdCat }</td>
				<td><a href="${cp}/board/faqdetail?brdNo=${dto.brdNo}">${dto.brdTitle }</a></td>
				<td>${dto.mgrId }</td>
				<td>${dto.brdWdate }</td>
			</tr>
		</c:forEach>
	...
	<div>
		<form method="post" action="${cp }/board/faqlist">
			<select name="field">
	...
	<button onclick="location.href='${cp }/board/faqinsert'">게시글 작성</button>
	<button onclick="location.href='${cp }/board/faqlist'">전체글 보기</button>
